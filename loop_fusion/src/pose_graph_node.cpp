...
void extrinsic_callback(const nav_msgs::Odometry::ConstPtr &pose_msg) {
  ...
  qic = Quaterniond(pose_msg->pose.pose.orientation.w, pose_msg->pose.pose.orientation.x, pose_msg->pose.pose.orientation.y, pose_msg->pose.pose.orientation.z).toRotationMatrix();
  m_process.unlock();
}
...
void process() {
  while (true) {
    ...
    if (pose_msg != NULL) {
      ...
      Matrix3d R = Quaterniond(pose_msg->pose.pose.orientation.w, pose_msg->pose.pose.orientation.x, pose_msg->pose.pose.orientation.y, pose_msg->pose.pose.orientation.z).toRotationMatrix();
      if ((T - last_t).norm() > SKIP_DIS)  //可另外设置回环距离
      {
...
void command() {
  while (1) {
    ...
    if (c == 'p') {
      m_process.lock();
      posegraph.savePoseGraphProto();
      m_process.unlock();
      printf("save pose graph Proto finish\nyou can set 'load_previous_pose_graph' to 1 in the config file to reuse it next time\n");
