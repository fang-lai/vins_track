...
void FeatureTracker::setMask() {
  if (FISHEYE_MASK_RAD) {
    if (fisheye_mask.empty()) {
      fisheye_mask = cv::Mat::zeros(row, col, CV_8UC1);
      cv::circle(fisheye_mask, cv::Point2f(row / 2, col / 2), FISHEYE_MASK_RAD,
                 cv::Scalar(255), -1, 8, 0);
      // cv::imwrite("/home/jon/code/ubt_code/mult_mono_vins/vins_mult_ws/src/VINS-Fusion/config/mask.jpg",
      // fisheye_mask);
    }
    mask = fisheye_mask.clone();
  } else if (!FISHEYE_MASK.empty()) {
    if (fisheye_mask.empty())
